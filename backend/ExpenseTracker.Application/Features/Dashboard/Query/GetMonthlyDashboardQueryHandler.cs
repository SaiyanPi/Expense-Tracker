...
public class GetMonthlyDashboardQueryHandler : IRequestHandler<GetMonthlyDashboardQuery, DashboardSummaryDto>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var totalExpenses = await _dashboardRepository.GetTotalExpensesForMonthAsync(request.UserId, startDate, endDate, cancellationToken);
        var totalBudgets = await _dashboardRepository.GetTotalBudgetForMonthAsync(request.UserId, startDate, endDate, cancellationToken);
        var expensesByCategory = await _dashboardRepository.GetExpensesByCategoryForMonthAsync(request.UserId, startDate, endDate, cancellationToken);
        var dailyExpenses = await _dashboardRepository.GetDailyExpensesForMonthAsync(request.UserId, startDate, endDate, cancellationToken);
        var recentExpenses = await _dashboardRepository.GetRecentExpensesForMonthAsync(request.UserId, startDate, endDate, 5, cancellationToken);
